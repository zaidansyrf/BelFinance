                                            ...
                                            @foreach ($Bills as $bill)
                                                <tr>
                                                    ...
                                                    <td class="flex items-center">
                                                        <a href="{{ route('sumber-keluar.edit', $bill->id) }}"
                                                            class="btn-primary bg-[#2B7A78] hover:bg-[#205C5D] px-3 py-1 rounded">Edit</a>
                                                        <form action="{{ route('sumber-keluar.destroy', $bill->id) }}"
                                                            method="POST"
                                                            onsubmit="return confirm('Apakah Anda yakin ingin menghapus data {{ $bill->name }}?');"
                                                            class="ml-2">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
                                                                class="bg-red-500 text-white px-3 py-1 rounded hover:underline">Hapus</button>
                                                        </form>
                                                    </td>
                                                </tr>
                                            @endforeach
                            ...
                            <form action="{{ route('sumber-keluar.store') }}" method="POST">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" id="sourceName"
                                        class="w-full p-2 border border-gray-300 rounded-md" name="name"
